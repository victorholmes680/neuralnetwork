...
// This idea is that it will spit out a binary file that can be then loaded up with nn.h and used in your application
...
// argv --
//          some1  some2  some3 ...
// char ***argv  --- &outer_argv(char **outer_argv)
char *args_shift(int *argc, char ***argv) {
    assert(*argc > 0);
    char *result = **argv; // (*argv)[0]
    (*argc) -= 1;
    (*argv) += 1;
    return result;
}
...
void nn_render_raylib(NN nn, int rx, int ry, int rw, int rh) {
    ...
    Color low_color = {0x00, 0xFF ,0x00, 0xFF};
	...
	for(size_t i = 0; i < nn.as[l].cols; ++i) {
	    int cx1 = nn.x + l*layer_hpad + layer_hpad/2;
	    int cy1 = nn.y + i*layer_hpad + layer_vpad1/2;
	    if(l+1 < arch_count) {
...
int main()
{
    ...
    // pass to the load function to accept the number of buffer len
    ...
    InitWindow(IMG_HEIGHT, IMG_HEIGHT, "gym");
    SetTargetFPS(60);
