...
typedef struct {
    size_t count;
    Mat *ws; // parameter of the model 
    Mat *bs; // bias of the neural layer
    Mat *as; // the result of each neural layer
} NN;
...
void mat_dot(Mat dst, Mat a, Mat b)
{
	...
	for(size_t j = 0; j < dst.cols; ++j) {
	    MAT_AT(dst, i, j) = 0; // initialize the dst with 0
...
void mat_save(FILE *out, Mat m)
{
	...
	// if one line elements have not been written completely, continue the logic
...
void nn_backprop(NN nn, NN g, Mat ti, Mat to)
{
    ...
    size_t n = ti.rows; // the number of sample for training
	...
	// get the input from ti to neural network nn
	mat_copy(NN_INPUT(nn), mat_row(ti, i));

	nn_forward(nn); // todo

	// this may be redundant since we have initialize the g before
	for(size_t j = 0; j <= nn.count; ++j) {
	    mat_fill(g.as[j], 0);
	}

	// calcualte the diff between the output of nn and to
	for(size_t j = 0; j < to.cols; ++j) {
	    MAT_AT(NN_OUTPUT(g), 0, j) = MAT_AT(NN_OUTPUT(nn), 0, j) - MAT_AT(to, i, j);
	}
	
	// y = w * pa + b
	// a = sigmoidf(y), the value derivative of sigmoidf is a*(1-a)
	// loss = (a' - a) ^ 2, we should assume a' is actual value and a is computed value
	// d|loss / d|b = 2*(a'-a)*(d|a / d|y) * (d|y / d|b)
	// d|loss / d|b = 2*diff*a*(1-a)*1
...
void gym_render_nn(NN nn, float rx, float ry, float rw, float rh)
{
    ...
    float layer_border_vpad = rh*0.08; // horizon pad
    float layer_border_hpad = rh*0.06; // vertical pad

    float nn_width = rw - 2*layer_border_hpad; // minus left and right pad
    float nn_height = rh - 2*layer_border_vpad; // minus top and bottom pad

    float nn_x = rx + rw/2 - nn_width/2; // rx + layer_border_hpad
    float nn_y = ry + rh/2 - nn_height/2; // ry + layer_border_vpad
